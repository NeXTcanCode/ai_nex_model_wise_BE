...
app.post('/api/v1/prompts/optimize', auth, async (req, res, next) => {
  try {
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.GROQ_API_KEY}` }, body: JSON.stringify({ model: process.env.GROQ_OPTIMIZER_MODEL || process.env.GROQ_MODEL, temperature: 0.1, messages: [{ role: 'system', content: 'Rewrite the user prompt to be clearer and more concise. Preserve intent, requirements, constraints, code, URLs, paths, commands, and quoted text. Do not answer the prompt. Return only the rewritten prompt.' }, { role: 'user', content: prompt }] }) });
