export const USAGE_LIMITS = Object.freeze({
  ...
  outputModes: Object.freeze({
    concise: Object.freeze({ maxOutputTokens: 800, instruction: "Answer directly and briefly." }),
    standard: Object.freeze({ maxOutputTokens: 1_500, instruction: "Provide a clear, balanced response." }),
    detailed: Object.freeze({ maxOutputTokens: 4_000, instruction: "Provide a comprehensive response with relevant detail." }),
  }),
});
...
export const weightedUnits = (inputTokens, outputTokens) =>
  Math.max(0, Number(inputTokens) || 0) + Math.max(0, Number(outputTokens) || 0) * 2;
...
export const usageSummary = (events, at = Date.now()) => {
  ...
  const recent = events.filter((event) => new Date(event.createdAt).getTime() >= weekCutoff);
  const weeklyUnits = recent.reduce((total, event) => total + (event.weightedUnits || 0), 0);
  const daily = recent.filter((event) => new Date(event.createdAt).getTime() >= dayCutoff);
  const minute = recent.filter((event) => new Date(event.createdAt).getTime() >= minuteCutoff);
  const dailyUnits = daily.reduce((total, event) => total + (event.weightedUnits || 0), 0);
  const resetAt = recent.length
    ? new Date(Math.min(...recent.map((event) => new Date(event.createdAt).getTime())) + 7 * 24 * 60 * 60 * 1000).toISOString()
    : null;

  return {
    usedUnits: weeklyUnits,
    limitUnits: USAGE_LIMITS.weeklyUnits,
    percent: Math.min(100, Math.round((weeklyUnits / USAGE_LIMITS.weeklyUnits) * 100)),
...
export const quotaError = ({ summary, estimatedInputTokens }) => {
  ...
  if (summary.usedUnits + estimatedInputTokens > USAGE_LIMITS.weeklyUnits)
    return "Not enough weekly usage remains for this prompt.";
  if (summary.dailyUsedUnits + estimatedInputTokens > USAGE_LIMITS.dailyUnits)
    return "Your daily usage allowance has been reached.";
  ...
  if (summary.requestsToday >= USAGE_LIMITS.requestsPerDay)
    return "Your daily message allowance has been reached.";
  if (summary.requestsThisWeek >= USAGE_LIMITS.requestsPerWeek)
    return "Your weekly message allowance has been reached.";
  return null;
};
